...
sir_asymptomatic <- function(time, state, parameters) {
  ...
  with(as.list(c(state, parameters)), {
    ...
    dA <- S*(beta1*I+ beta2*A )*percent_asymptomatic - gamma1*A
    dI <-   S * (beta1*I+ beta2*A ) *(1-percent_asymptomatic) - gamma2 * I
    dI_new <-    S * (beta1*I) 
...
parameters_asmyptomatic <- c(beta1 = .95,beta2=2.1, gamma1 = .75,gamma2=.52,percent_asymptomatic=.25)
## Time frame
...
## beta: infection parameter; gamma: recovery parameter
parameters <- c(beta1 = .95,beta2=2.1, gamma1 = .75,gamma2=.52,percent_asymptomatic=.25)
## Time frame
times      <- seq(0, 70, by = 1)
